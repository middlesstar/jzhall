...
void BlackFish::OnUpdate(float dt)
{
	...
	const MovePoints& paths = PathManager::instance()->getPaths(SMALL_FISH_PATH, m_wPathID);
	if (paths.empty())
		return;
	MOVE_POINT mp;
...
bool BlackFish::InsideScreen()
{
	...
	return m_Position.x > 20.f && m_Position.x < GameUtils_Shark::winSize.width*scaleX - 20.0f && m_Position.y > 20.f && m_Position.y < GameUtils_Shark::winSize.height*scaleY - 20.0f;
	//    return true;
}
